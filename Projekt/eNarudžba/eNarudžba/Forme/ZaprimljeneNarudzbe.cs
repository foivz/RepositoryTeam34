    ...
    public partial class ZaprimljeneNarudzbe : Form
    {
        ...
        private void PrikaziZaprimljeneNarudzbe() 
        {
            using (T34_DBEntities2 db = new T34_DBEntities2())
            {
                                                                                                                                                                               ...
                                                                                                                                                                               //ns.Gotovo!=5
                var upit = (from n in db.Narudzba join k in db.Korisnik on n.IDnarucitelj equals k.OIB join ns in db.NarudzbaStatus on n.IDnarudzba equals ns.IDnarudzba where ns.IDstatus!=5 group new { n, k } by new { n.IDnarudzba, n.DatumVrijemeZaprimanja, n.CijenaNarudzbe, k.Ime, k.Prezime, k.Adresa } into grup select new { grup.Key.IDnarudzba, grup.Key.DatumVrijemeZaprimanja, Cijena = grup.Sum(n => n.n.CijenaNarudzbe), grup.Key.Ime, grup.Key.Prezime, grup.Key.Adresa }).OrderByDescending(n => n.IDnarudzba).ToList();
